...
import { FiMapPin, FiPhoneIncoming, FiBookOpen , FiMail, FiCalendar, FiBook, FiLinkedin} from "react-icons/fi";
...
const About = ( ) => {
    const aboutQueryData = useStaticQuery(graphql`
        query AboutDefaultQuery {
            ...
            file(relativePath: {eq: "images/banner/margot-w.jpeg"}) {
              childImageSharp {
                gatsbyImageData(
                  width: 395
                  height: 470
                                          ...
                                          if(value.link.startsWith('https:')){
                                            return (
                                                <div className="list" key={index}>
                                                    ...
                                                    <a className="link" href="#labelvalue" target="_blank">{value.link}</a>
                                                </div>
                                            )
                                          }else{
                                            return (
